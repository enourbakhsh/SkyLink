...
import numpy as np
from astropy.coordinates.representation import UnitSphericalRepresentation ## from .representation import UnitSphericalRepresentation
from astropy import units as u
from astropy.coordinates import Angle ## from . import Angle
...
# elif session.viewedonscreen(): # not here, it might be a forked process that wrongly says not viewed on screen
#     from tqdm import tqdm
# else:
#     print('Progressbars disabled.')
#     tqdm = dummy_tqdm
...
def search_around_sky(coords=None, coords1=None, coords2=None, seplimit=None, storekdtree='kdtree_sky', verbose=1, show_progress=True, silent=False, return_dists=False, tqdm_kwargs={}):
    """
    ...
    verbose : int, optional
        If 0, it does not write any words to stdout while running (it will still show the
        progressbars and busy indicators whenever needed)
        If 1, it writes some extra information about the status of the job
    show_progress : bool, optional
        If True, it will show the progressbars and busy indicators whenever needed.
        If False, it hides them. Except, in the case of busy indicators it will only hide the
        animation part not the message.
    silent : bool, optional
        If True, it supresses the stdout, i.e. it sets `verbose=0` and `show_progress=False` plus it
        hides the busy indicators alltogether along with any message they might have.
        `silent` overrides `verbose` and `show_progress`.
    return_dists : bool, optional
        If True, it returns sep2d and dist3d in addition to the matching indices (i.e. idx1, idx2)
    ...
    if coords is not None:
        if coords1 is not None or coords2 is not None:
            raise ValueError('either use `coords` for internal matching or `coords1` and `coords2` for cross matching.')
        coords1 = coords
        coords2 = coords
    ...
    with BusyPal('Growing the first KD-Tree', style={'id':6,'color':'sandy_brown'}, fmt='{spinner} {message}', skip=skip_busypal):
    ...
    if storekdtree and coords2.cache.get(storekdtree):
        ...
        if same_coords:
            kdt2 = kdt1
            if verbose: print(cl.stylize('✔', cl.fg('green')+cl.attr('bold'))+' Used the same KD-Tree for the second set of coordinates since this is an internal match.')
        else:
            # strip out distance info
            urepr2 = coords2.data.represent_as(UnitSphericalRepresentation)
            ucoords2 = coords2.realize_frame(urepr2)
            with BusyPal('Growing the second KD-Tree', style={'id':6,'color':'SANDY_BROWN'}, fmt='{spinner} {message}', skip=skip_busypal):
        ...
        if storekdtree:
            ...
            if verbose:
                print(cl.stylize('✔', cl.fg('green')+cl.attr('bold'))+' Stored the KD-Tree in the cache.') # as `{storekdtree}`.')
    ...
    with BusyPal(f'Finding all pairs of points whose distance is at most {seplimit}', style={'id':6,'color':'sandy_brown'}, fmt='{spinner} {message}', skip=skip_busypal):
        found_pairs = kdt1.query_ball_tree(kdt2, r)

    # if not disable_tqdm: # for multiprocessing
    #     print(' ', end='', flush=True) ## for tqdm to work in jupyter: https://github.com/tqdm/tqdm/issues/485#issuecomment-473338308
    ...
    for i, matches in enumerate(tqdm(found_pairs, disable=disable_tqdm, **tqdm_kwargs)):
        for match in matches:
            ...
            idxs2.append(match)
    if verbose:
        print('\r\r'+cl.stylize('✔', cl.fg('green')+cl.attr('bold'))+' Created matching lists from KD-Trees')
